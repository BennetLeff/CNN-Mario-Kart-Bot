...
def spatial_argmax(logit):
    ...
    a = (weights.sum(1) * torch.linspace(-1, 1, logit.size(2)).to(logit.device)[None]).sum(1)
    b = (weights.sum(2) * torch.linspace(-1, 1, logit.size(1)).to(logit.device)[None]).sum(1)
...
class Planner(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        return spatial_argmax(self.classifier(z).squeeze())
